...
export default function InviteQuote({ onCounts }: { onCounts?: (counts: Record<string, number>) => void }) {
  ...
  // Invite Quote is a bidding flow — mechanics compete via bids, so its pills
  // track the bid lifecycle instead of a single mechanic dispatch.
  const filters: FilterOption[] = [
    { label: 'All', key: null },
    { label: 'Requested', key: 'requested' },
    { label: 'Bid Live', key: 'bid live' },
    { label: 'Bid Ended', key: 'bid ended' },
    { label: 'Mechanic Selected', key: 'mechanic selected' },
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '0', margin: '0 -1.5rem -1.5rem -1.5rem' }}>
        <OrdersTable activeTab="Invite Quote" activeFilter={activeFilter} onCounts={onCounts} onFilterCounts={setFilterCounts} />
