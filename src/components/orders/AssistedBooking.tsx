...
export default function AssistedBooking({ onCounts }: { onCounts?: (counts: Record<string, number>) => void }) {
  ...
  // Assisted Booking is a direct, admin-assisted mechanic dispatch — same
  // ground-service lifecycle as Instant Smart Booking, no remote diagnosis step.
  const filters: FilterOption[] = [
    { label: 'All', key: null },
    { label: 'Mechanic Allotted', key: 'mechanic allotted' },
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '0', margin: '0 -1.5rem -1.5rem -1.5rem' }}>
        <OrdersTable activeTab="Assisted Booking" activeFilter={activeFilter} onCounts={onCounts} onFilterCounts={setFilterCounts} />
