              ...
              <div style={{ padding: '1.5rem', backgroundColor: 'white', borderTop: '1px solid #f3f4f6', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                    ...
                    <div className="form-field">{order.paymentMethod}</div>
                  </div>
                  
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                    <label style={{ fontSize: '0.75rem', color: '#9ca3af', fontWeight: 600 }}>Transaction ID</label>
                    <div className="form-field">{order.txnId}</div>
                  </div>
                </div>
            ...
            {isTimelineExpanded && (
              <div style={{ padding: '1.5rem', backgroundColor: 'white', borderTop: '1px solid #f3f4f6' }}>
                ...
                <div style={{ paddingLeft: '0.5rem' }}>
                  {[
                    { title: 'Order Received', desc: 'Order placed by Aditya Bhargav', date: "10:30 PM, 21 Jan' 26", completed: true },
                    { title: 'Processing', desc: 'Order details verified and spare parts packaged', date: "09:15 AM, 22 Jan' 26", completed: order.status !== 'Order Received' },
                    { title: 'Shipped', desc: 'Package handed over to shipping courier', date: "03:40 PM, 22 Jan' 26", completed: ['Shipped', 'Out for Delivery', 'Delivered'].includes(order.status) },
                    { title: 'Out for Delivery', desc: 'Package out with delivery agent in New Delhi', date: "11:00 AM, 23 Jan' 26", completed: ['Out for Delivery', 'Delivered'].includes(order.status) },
                    { title: 'Delivered', desc: 'Package delivered at shipping address', date: "02:25 PM, 23 Jan' 26", completed: order.status === 'Delivered' }
                  ].map((node, index) => (
